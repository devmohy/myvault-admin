...
const { activateBusiness, deactivateBusiness, approveBusiness, declineBusiness,blacklistBusiness, isLoading } = useBusinesses();
...
const handleApprove = async () => {
  ...
  await approveBusiness(props.businessId, data);
  setTimeout(() => {
    isLoading.value = false;
    window.location.reload();
  }, 1000);
...
const handleDecline = async () => {
  ...
  await declineBusiness(props.businessId, data);
  reason.value= ""
  setTimeout(() => {
    isLoading.value = false;
    window.location.reload();
...
const handleBlackList = async () => {
  ...
  await blacklistBusiness(props.businessId, data);
  setTimeout(() => {
    isLoading.value = false;
    window.location.reload();
  }, 1000);
    ...
    <div class="relative w-full max-w-md max-h-full">
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
        ...
        >
          ...
          >
            ...
            <span class="sr-only">Close modal</span>
                ...
                <div
          v-if="actionWord == 'Deactivate'|| actionWord == 'Blacklist'"
          class="px-4 md:px-5 py-2 space-y-4"
        >
          <p class="flex text-sm text-[#475467] dark:text-gray-400">
            By {{ actionWord == 'Deactivate' ? 'deactivating' : 'blacklisting' }} this business, the corresponding business
            will no longer have access to the platform features.
          </p>
        </div>
        <div v-if="actionWord == 'Decline' || actionWord == 'Deactivate'" class="px-4 md:px-5 py-2 space-y-4">
          <div>
            <label
              for="message"
            ...
            <textarea
              ...
              :placeholder="`Enter the reason for choosing to ${actionWord.toLowerCase()} this business`"
            ></textarea>
          </div>
        </div>
        <div
          v-if="actionWord == 'Approve' || actionWord == 'Activate'"
          class="px-4 md:px-5 py-2 space-y-4"
        >
          <p class="flex text-sm text-[#475467] dark:text-gray-400">
            By {{ actionWord == 'Approve' ? 'approving' : 'activating' }} this business, the corresponding business
            will have access to the platform features.
          </p>
        </div>
        <div v-if="actionWord == 'Remove'" class="px-4 md:px-5 py-2 space-y-4">
          <p class="flex text-sm text-[#475467] dark:text-gray-400">
            You are about to remove this employee from this payroll. Kindly confirm this
            action
          </p>
        </div>
        <div
        ...
        >
          ...
          >
            {{isLoading ? 'Please wait...' : actionWord+' business'}}
          </button>
          <button
          :disabled="isLoading || reason == ''"
            v-if="actionWord == 'Decline'"
          ...
          <button
          :disabled="isLoading"
            v-if="actionWord == 'Blacklist'"
            @click.prevent="handleBlackList"
            data-modal-hide="approve-decline-modal"
            type="button"
            class="text-[#FFFFFF] bg-[#D92D20] focus:ring-2 focus:outline-none focus:ring-b[#D92D20] font-medium rounded-full text-sm px-5 py-2.5 text-center dark:bg-[#D92D20] dark:hover:bg-[#D92D20] dark:focus:ring-[#D92D20]"
          >
